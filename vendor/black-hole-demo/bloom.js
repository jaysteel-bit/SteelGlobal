// Steel Global: zero-weight bloom stub. NNLS solve in bloom_filters_generator
// fails on Ubuntu 24.04 / libeigen3 3.4.0; we don't ship bloom anyway.
// MAX_LEVELS = 9 below; provide a 9-entry inner array so resize() doesn't
// dereference undefined when iterating filters[i][index].
const BLOOM_FILTERS = [
  600,
  Array.from({length: 9}, () => [0, 0, 0, 0, 0, 0]),
];
...
const RENDER_SHADER =
  ...
  void main() {
    ...
    float alpha = texture(source, source_uv).a;
...
// Steel Global: expose Bloom on BlackHoleShaderDemoApp for camera_view.js to consume.
