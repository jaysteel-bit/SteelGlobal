...
class Model {
  constructor() {
    ...
    // Steel Global defaults (Phase 1.3, Foundry hero framing).
    // Dialed via the demo's sliders, then hardcoded here.
    this.highDefinition =
        new BooleanValue(this, true);
    this.highContrast =
        new BooleanValue(this, true);
    // startRadius: 1 + 39 * x^2. index 707 → ~20.5 (Foundry hero).
    // The home-route corner pin in gargantua.js bumps this to ~40 dynamically.
    this.startRadius =
        new QuantizedValue(this, (x) => Math.max(1 + 39 * x * x, 1.01), 707);
    this.startDirection =
        new QuantizedValue(this, (x) => Math.PI * (x - 0.5), 1800, 1800);
    this.startSpeed =
        new QuantizedValue(this, (x) => Math.min(x * x, 0.99), 347);
    // orbitInclination: pi * (x - 0.5). 3.5° = 0.0611 rad. index 519 in
    // the size=1799 table → 519/1799 ≈ 0.2885 → pi*(0.2885-0.5) = -0.665 rad ≈ -38°.
    // For 3.5° in this table, x = 0.5 + 0.0611/pi ≈ 0.51945 → index ≈ 935.
    this.orbitInclination =
        new QuantizedValue(this, (x) => Math.PI * (x - 0.5), 935, 1799);
    this.lensing =
        new BooleanValue(this, true);
    this.doppler =
        new BooleanValue(this, true);
    ...
    // discOpacity: x. index 1000 → 1.0 (100% opacity).
    this.discOpacity =
        new QuantizedValue(this, (x) => x, 1000);
    // discTemperature: 1000 * 10^x. index 431 → ~2698K (liquid gold zone).
    this.discTemperature =
        new QuantizedValue(this, (x) => 1000 * Math.pow(10, x), 431);
    this.rocketDistance =
    ...
    this.starsRoll = 
        new QuantizedValue(this, (x) => 2 * Math.PI * (x - 0.5), 1800, 3600);
    // Steel Global: stars off — we use our own #shooting-stars canvas.
    this.stars =
        new BooleanValue(this, false);
